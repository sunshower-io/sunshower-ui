...
public class HFSClient {
    ...
    public HFSClient(String host, int port) {
        this(host, port, "/api/v0/");
    }

    public HFSClient(String multiaddr) {
        this(new MultiAddress(multiaddr));
    }

    public HFSClient(MultiAddress addr) {
        this(addr.getHost(), addr.getTCPPort(), "/api/v0/");
    }

    public HFSClient(String host, int port, String version) {
        this.host = host;
        this.port = port;
    ...
    private InputStream retrieveStream(String path) throws IOException {
        URL target = new URL("http", host, port, version + path);
        return HFSClient.getStream(target);
    }
