...
export class ParallelSchedule<T> implements Traversal<Sequence<T>[], T> {
    ...
    run(g: Graph<T>): Sequence<T>[] {
        let graph = this.pluck(copy(
            ...
            keyspace = this.computeKeyspace(graph)
                .filter(ks => graph.nodes[ks.id] == null);
    ...
    private computeKeyspace(g: Graph<T>): Vertex<T>[] {
        return _.uniqBy(_.values<Vertex<T>>(g.nodes)
            .concat(
                _.flatMap(
                    _.values<Vertex<T>>(g.nodes),
                    node => _.values<Edge<T>>(node.adjacencies).map(a => a.target)
                )), e => e.id);
    }


    public roots(nodes: {[p: string]: Vertex<T>},
                 existing?: {[p: string]: boolean}): Vertex<T>[] {
        existing = existing || {};
        let adjacentNodes = _.reduce(_.flatMap<Vertex<T>>(
            _.values(nodes),
            n => _.values<Edge<T>>(n.adjacencies).map(a => a.source)
        ), (u, t) => {
            u[t.id] = t;
            return u;
        }, new Map<string, Vertex<T>>());
        return _.values<Vertex<T>>(nodes).filter(
            n => adjacentNodes[n.id] == null &&
            existing[n.id] == null
        );
    }
}
