...
export class Components<T> {
    ...
    getCycles(): Component<T>[] {
        return _.filter(this.components,
            (component: Component<T>) => component.isCycle()
        );
    }

    sort(): Vertex<T>[] {
        return _.reduce(this.components,
            (vs:Vertex<T>[], v:Component<T>) => {
                return _.reduce(v.elements, (ves, ve) => {
                    ves.unshift(ve);
                    return ves;
                }, vs);
            }, []);
